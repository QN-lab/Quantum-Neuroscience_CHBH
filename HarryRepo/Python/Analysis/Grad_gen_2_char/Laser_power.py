...
core_path = r'W:\Data\2025_02_26_laser_power\2'

#Cell 1: r'W:\Data\2025_02_26_laser_power\2'
#Cell 2: r'W:\Data\2025_02_28_Cell2_laser_power\1'
...
for _, _, folder_name in folder_data_pairs:
    ...
    initial_guess = [max(x_mean), 2000, 100, 0, min(x_mean)]
...
# Extract power values from folder names
power_values = []
folder_order = []
power_ratio = 0.069264 #0.1008

for _, _, folder_name in folder_data_pairs:
    match = re.search(r"(\d+)uW", folder_name)
    if match:
        power_values.append(int(match.group(1)) * power_ratio)  # Multiply by power ratio
        folder_order.append(folder_name)

# Sort by extracted power values
sorted_indices = np.argsort(power_values)
power_values = np.array(power_values)[sorted_indices]
sorted_folder_names = np.array(folder_order)[sorted_indices]

# Extract relevant data for sorted runs
widths = [fitted_params_dict[folder_name][2] for folder_name in sorted_folder_names]
amplitudes = [fitted_params_dict[folder_name][0] for folder_name in sorted_folder_names]
amplitude_width_ratios = [ratio_dict[folder_name] for folder_name in sorted_folder_names]
phase_slopes = [slope_fit_dict[folder_name] for folder_name in sorted_folder_names]  # Use fitted phase slope
...
save_dir =  r'W:\Data\2025_02_26_laser_power' # Change this to your desired directory
os.makedirs(save_dir, exist_ok=True)  # Ensure the directory exists

# Save the data
save_path = os.path.join(save_dir, "Cell1_laser.npz")
np.savez(save_path, 
