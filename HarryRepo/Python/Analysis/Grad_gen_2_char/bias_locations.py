...
core_path = r'W:\Data\2025_03_04_bias_location\1'
...
for _, _, folder_name in folder_data_pairs:
    ...
    initial_guess = [max(x_mean), np.mean(freq_mean), 100, 0, min(x_mean)]
...
for _, _, folder_name in folder_data_pairs:
    match = re.search(r"(\d+\.\d+)kHz", folder_name)
    if match:
        power_values.append(float(match.group(1)))  # Multiply by power ratio
...
axes[0].set_title("Width vs bias location")
axes[0].grid(True)

#
axes[1].plot(power_values, amplitudes, marker='o', linestyle='-', color='b')
axes[1].set_ylabel("Amplitude (nA)")
axes[1].set_title("Amplitude vs bias location")
axes[1].grid(True)

# Plot amplitude/width ratio against power values
axes[2].plot(power_values, phase_slopes, marker='s', linestyle='-', color='b')
axes[2].set_ylabel("Phase slope (degs/Hz)")
axes[2].set_title("Phase slope vs bias location")
axes[2].grid(True)
