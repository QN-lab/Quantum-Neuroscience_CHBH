...
core_path = r'W:\Data\2025_02_27_heating\1'
...
for _, _, folder_name in folder_data_pairs:
    match = re.search(r"(\d+)C", folder_name)
    if match:
        power_values.append(int(match.group(1)))  # Multiply by power ratio
...
axes[0].set_title("Width vs Heating")
axes[0].grid(True)

#
axes[1].plot(power_values, amplitudes, marker='o', linestyle='-', color='b')
axes[1].set_ylabel("Width(Hz)")
axes[1].set_title("Amplitude vs Heating")
axes[1].grid(True)

# Plot amplitude/width ratio against power values
axes[2].plot(power_values, amplitude_width_ratios, marker='s', linestyle='-', color='r')
...
axes2[0].set_title("Phase slope vs Heating")
axes2[0].grid(True)

axes2[1].plot(power_values, np.array(phase_slopes)/np.array(widths), marker='^', linestyle='-', color='b')
axes2[1].set_xlabel("Temperature (C)")
axes2[1].set_ylabel("Phase-per-width (degs/Hz/Hz)")
axes2[1].set_title("Phase slope per width")
axes2[1].grid(True)
...
save_dir =  r'W:\Data\2025_02_27_heating' # Change this to your desired directory
os.makedirs(save_dir, exist_ok=True)  # Ensure the directory exists

# Save the data
save_path = os.path.join(save_dir, "Cell1_heat.npz")
np.savez(save_path, 
