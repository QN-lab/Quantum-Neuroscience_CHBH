...
core_path = r'W:\Data\2025_03_10_gradiometer_measure_gradient\res'
...
for header_file, sample_file, folder_name in folder_data_pairs:
    ...
    auxin0_inds = sample_file.index[sample_file['fieldname'] == 'auxin0'].tolist()  # Extract phase_ data
    ...
    PD_data = sample_file.iloc[auxin0_inds, 4:-1].to_numpy()

    #Translate PD data to cell power data

    cell_power_data = np.exp((PD_data*1000-209.3080)/41.31)
    #cell_power_data = PD_data
    ...
    cell_power_mean = np.mean(cell_power_data, axis=0)
    cell_power_std = np.std(cell_power_data, axis=0)
    ...
    cell_power_data_dict[folder_name] = (cell_power_mean, cell_power_std)  # Store phase data
...
for _, _, folder_name in folder_data_pairs:
    ...
    initial_guess = [max(x_mean), 1500, 100, 0, min(x_mean)]
    ...
    plt.plot(freq_mean, fitted_x, label=f'(Amp={amp_nano:.2f} nA, Width={wid:.2f}, Ratio={amp_to_width_ratio:.2f})')
...
for idx, (_, _, folder_name) in enumerate(folder_data_pairs):
    ...
    plt.plot(freq_range_raw, phase_range_raw, label=f'Raw Phase')
    ...
    slope_fit_dict[folder_name] = -1*slope_fit  # Store in dictionary
    ...
    plt.plot(freq_range_fit, fitted_phase, line_style, label=f' Fit (Slope={slope_fit:.3f})')
...
#%% Plot width, amplitude/width ratio, and phase slope against extracted power values

# Extract relevant data
widths = [fitted_params_dict[folder_name][2] for _, _, folder_name in folder_data_pairs]
amplitudes = [fitted_params_dict[folder_name][0] for _, _, folder_name in folder_data_pairs]
amplitude_width_ratios = [ratio_dict[folder_name] for _, _, folder_name in folder_data_pairs]
phase_slopes = [slope_fit_dict[folder_name] for _, _, folder_name in folder_data_pairs]  # Use fitted phase slope


power_values = [np.mean(cell_power_data_dict[folder_name][0]) for _, _, folder_name in folder_data_pairs]

# Create subplots
fig, axes = plt.subplots(3, 1, figsize=(8, 12))

# Plot width against power values
axes[0].plot(power_values, widths, marker='o', linestyle='-', color='b')
axes[0].set_ylabel("Width(Hz)")
axes[0].set_title("Width vs Power")
axes[0].grid(True)

# Plot amplitude/width ratio against power values
axes[1].plot(power_values, amplitudes, marker='s', linestyle='-', color='r')
axes[1].set_ylabel('Amplitudes (nA)')
axes[1].set_title("Amplitude vs Power")
axes[1].grid(True)

# Plot fitted phase slope against power values
axes[2].plot(power_values, amplitude_width_ratios, marker='^', linestyle='-', color='g')
...
# Plot fitted phase slope against power values
axes2[0].plot(power_values, phase_slopes, marker='^', linestyle='-', color='g')
...
axes2[1].plot(power_values, np.array(phase_slopes), marker='^', linestyle='-', color='g', label="Phase Slope")
axes2[1].set_xlabel("Assumed power before first cell (uW)")
axes2[1].set_ylabel("Phase slope (degs/Hz)", color='g')
axes2[1].set_title("Phase slope against width")
axes2[1].grid(True)

# Create a second y-axis sharing the same x-axis
ax2 = axes2[1].twinx()
ax2.plot(power_values, np.array(widths)/2, marker='o', linestyle='--', color='r', label="Half-Widths")
ax2.set_ylabel("Half-Width (Hz)", color='r') 
...
#save_dir =  r'W:\Data\2025_03_06_grad_power' # Change this to your desired directory
#os.makedirs(save_dir, exist_ok=True)  # Ensure the directory exists

# Save the data
#save_path = os.path.join(save_dir, "Grad_power2.npz")
#np.savez(save_path, 
#         power_values=power_values,
#         widths=widths,
#         amplitudes=amplitudes,
#         amplitude_width_ratios=amplitude_width_ratios,
