...
source_file = '_f.csv'
output_file = '_g_r.csv'




source_file_2 = '_fl.csv'

output_file_2 = '_fl_r.csv'
...
class Data:
    ...
    def compute(self, Trig_in2_idx, Aux1_v_idx):
        for chunk in self.chunks:
            if chunk.compute(Trig_in2_idx, Aux1_v_idx):
                self.good_chunks.append(chunk.number)
            else:
                self.bad_chunks.append(chunk.number)

    def to_print(self):
        for chunk in self.chunks:
            chunk.to_print()

    def print_good_chunks(self):
        for chunk in self.good_chunks:
            print(f'{chunk}')
...
def read_file_csv(file):
    ...
    #print(f"Input1: {input1_idx}")
    data2 = []
    chunk_number = -1
    in_chunk = False
    for row in reader:
        if float(row[input1_idx]) == 2:
            if not in_chunk:
                chunk_number = chunk_number+1
                in_chunk = True
            if str(chunk_number) in data.good_chunks:
                row.append(5)# <- here
            else:
                row.append(0)
        else:
