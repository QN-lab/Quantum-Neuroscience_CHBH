...
plot_title='AER '+str(mode)+' (SF='+str(scal_fac)+', P='+str(P)+'m, I=100u, BW='+str(BW)+'), ' +'\n'+str(dir_name)
...
reject_criteria = dict(mag=19.5e-10)
reject_criteria_f = dict(mag=30e-10)
...
raw_g.copy().pick_types(meg=True, stim=True).plot(events=events_g, start=100, duration=10, scalings=dict(mag=1e-10),  color=dict(mag='darkblue', stim='gray'), event_color={ 3:'b'})
...
g_epochs_DF.iloc[:,:].plot(legend=None, ylim=[-2e-11,2e-11],title='our sensor'+str(mode) )
plt.show()
...
fl_epochs_DF.iloc[:,:].plot(legend=None, ylim=[-2e-11,2e-11],title='FL '+str(mode_fl))
plt.show()
...
color = 'tab:blue'
ax2.set_ylabel('FL '+mode_fl+' [T]', color=color)  # we already handled the x-label with ax1
...
fft_gf=np.sqrt(psds_gf)
psds_gf_mean = fft_gf.mean(axis=axis)[freq_range_g]
psds_gf_std = fft_gf.std(axis=(0, 1))[freq_range_g]
...
ax1.set_ylabel('FFT '+mode+' fT/sqrt(Hz)', color=color)
ax1.set_yscale('log')
ax1.set_xscale('log')
ax1.fill
...
color = 'tab:blue'
ax2.set_ylabel('FFT FL '+mode_fl+' fT/sqrt(Hz)', color=color)  # we already handled the x-label with ax1
ax2.set_yscale('log')
...
color = 'tab:blue'
ax2.set_ylabel('FL '+mode_fl+' [dB]', color=color)  # we already handled the x-label with ax1
...
fig_trf_fl = power_flf.plot(picks_fl,title= plot_title+' TFR FL '+mode_fl)
