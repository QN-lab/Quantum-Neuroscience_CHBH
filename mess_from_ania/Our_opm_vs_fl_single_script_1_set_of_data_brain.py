...
plot_title='Brain Grad +old FL'+' (SF='+str(scal_fac)+', P='+str(P)+'m, I=2m, BW='+str(BW)+'), ' +'\n'+str(dir_name)
...
data_fl.plot(x='time', y=['FL0104-BZ_CL','FL0104-BZ_CL']),#ylim=(-1e-11,+1e-11))
plt.show()
...
reject_criteria = dict(mag=10e-11)
reject_criteria_f = dict(mag=8e-11)
l_freq_r = 0.01
h_freq_r = 100
l_freq = 1
h_freq = 45
...
epochs_aux=mne.Epochs(raw_gf, picks=['eeg'], events=events_g, event_id=3, tmin=tmin_e, tmax=tmax_e,baseline=baseline,  preload=True).filter(l_freq=l_freq,h_freq=h_freq)
...
events_m_fl = mne.find_events(raw_fl, stim_channel=['Input1'], min_duration=0.01,   mask_type='not_and', mask=(pow(2,8)+pow(2,9)+pow(2,10)+pow(2,11)+pow(2,12)+pow(2,13)),output='onset',consecutive=True)

#raw_fl.copy().pick_types(meg=True, stim=True).plot(start=50, duration=100, scalings=dict(mag=1e-10),  color=dict(mag='darkblue', stim='gray'))
...
epochs_flf=mne.Epochs(raw_flf,events=events_m_fl, picks='mag', tmin=tmin_e,tmax=tmax_e, baseline =baseline, reject=reject_criteria_f,preload=True).filter(l_freq=l_freq,h_freq=h_freq)
evoked_flf=epochs_flf.copy().average(method='mean').filter(l_freq=l_freq,h_freq=h_freq)
N_events_fl = len(epochs_flf)#events_m_fl.shape[0]
#
evoked_flf.plot()
...
textbox='lp='+str(l_freq)+', hp='+str(h_freq)+', g_sd='+str(g_s_dev)+', fl_sd='+str(fl_s_dev)+', Ng='+str(N_events)+', Nfl='+str(N_events_fl)
print(textbox)
#%%
...
#fig_trf_g.savefig(str(dir_name) + '_trf_g.png')
...
#fig_trf_fl.savefig(str(dir_name) + '_trf_fl.png')
