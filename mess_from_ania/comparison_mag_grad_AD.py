...
grad_directory='Z:\\Data\\2023_08_14_bench\\long_runs\\grad_long_run_drift_000' # MAKE SURE THERE IS NO \\ AT THE END
mag_directory='Z:\\Data\\2023_08_14_bench\\long_runs\\mag_long_run_drift_000' # MAKE SURE THERE IS NO \\ AT THE END
save_directory='Z:\\Data\\2023_08_14_bench\\Analysis\\'
...
data_m.iloc[0:1000].plot(x=ch_names[0],y=ch_names[2:6])
plt.show()
print(data_m.head(1))
...
x1=at.Dataset(data=data_g['time'])

y_g=data_g['B_T_cal']-data_g.iloc[0][5]
signal_g=at.Dataset(data=y_g)


#print(len(x1), len(y_g))

#y1=data_g['value']
x2=data_m['time']-data_m.iloc[0][0]
y_m=data_m['B_T_cal']-data_m.iloc[0][5]
signal_m=at.Dataset(data=y_m)

t = np.logspace(-2, 6, 200)


(tg, ad, ade, adn) = at.oadev(y_g, rate=sfreq_g, data_type="phase", taus=t)  # Compute the overlapping ADEV
whiteN=at.noise.white(10000, b0=1e-26, fs=sfreq_g)
(tgw, adw, adew, adnw)=at.oadev(whiteN, rate=sfreq_g, data_type="phase", taus=t)

(t_m, ad_m, ade_m, adn_m) = at.oadev(y_m, rate=sfreq_m, data_type="phase", taus=t) 
...
st=0
sp=50
coeff_g= np.polyfit(np.log(tg[st:sp]),np.log(ad[st:sp]), 1 )
print(coeff_g)
coeff_w= np.polyfit(np.log(tgw),np.log(adw), 1 )
#fit_g=coeff_g[0]*tg+coeff_g[1]

fig_fft_adev, ax = plt.subplots()
ylim=[1e-14,1e-9]
color = 'tab:red'
ax.set_xlabel('Averaging time (s)')
ax.set_ylabel('Allan Deviation (T)', color='black')

ax.set_yscale('log')
ax.set_xscale('log')
ax.fill
ax.set_title('Allan deviation ' +str(dir_name))

ax.plot(tg, ad, color=color, label = 'Gradiometer')
ax.plot(tgw,adw, color='yellow', label = 'Theoretical white noise')
#ax1.plot(tg, np.log10(slope_g(np.log10(tg))), color='pink')
ax.plot(tg, np.exp(coeff_g[1])*tg**coeff_g[0], color='pink', label = 'Gradiometer fit')
ax.errorbar(tg, ad, yerr=ade, fmt ='.k', linewidth=1, color=color)
#ax1.fill_between(freqs_gf[freq_range_g], psds_gf_mean - psds_gf_std, psds_gf_mean + psds_gf_std,     color=color, alpha=.2)
ax.tick_params(axis='y', labelcolor='black')
ax.set(ylim=ylim)
ax.axvline(tg[st], color="black", linestyle="--", lw=0.5, alpha=1)
ax.axvline(tg[sp], color="black", linestyle="--", lw=0.5, alpha=1)
ax.legend( loc=0)
...
ax.plot(t_m, ad_m,  color=color, label = 'Magnetometer')
ax.errorbar(t_m, ad_m, yerr=ade_m, fmt ='.k', color=color)
...
color = 'tab:blue'
ax2.set_ylabel('Reference '+mode+' [T]', color=color)  # we already handled the x-label with ax1
...
epochs_g0f=mne.Epochs(raw_gf,picks=['mag'], events=events_g, event_id=3, tmin=tmin_e, tmax=tmax_e,baseline=baseline, reject=reject_criteria, preload=True)#.filter(l_freq=l_freq,h_freq=h_freq)
evoked_g0f=epochs_g0f.copy().average(method='mean').filter(l_freq=l_freq,h_freq=h_freq)

N_events=len(epochs_g0f)#or events_gf.shape[0]
print('number of left events is: ',N_events)
#evoked_gf.plot()
...
epochs_m0f=mne.Epochs(raw_mf,events=events_m, picks=['mag'], event_id=3, tmin=tmin_e,tmax=tmax_e, baseline =baseline, reject=reject_criteria,preload=True)#.filter(l_freq=l_freq,h_freq=h_freq)
evoked_m0f=epochs_m0f.copy().average(method='mean').filter(l_freq=l_freq,h_freq=h_freq)

N_events_m = len(epochs_m0f)#events_m_m.shape[0]
print('number of left events is: ',N_events_m)
#
#evoked_mf.plot()
...
%%
##### plotting
g0_epochs_data=np.squeeze(epochs_g0f.get_data())
g0_epochs_data.shape
g0_epochs_DF=pd.DataFrame(data=(g0_epochs_data.T))
...
g0_epochs_DF.iloc[:,:].plot(legend=None, ylim=[-2e-11,2e-11],title='gradiometer' )
plt.show()
#g_epochs_DF.to_csv("epochs_origin.csv", sep=',', index=False)
#g_av=g_epochs_DF.iloc[:,350:].mean(axis=1)

g0_av=g0_epochs_DF.mean(axis=1)
g0_std=g0_epochs_DF.std(axis=1)
g0_sem=g0_epochs_DF.sem(axis=1)
g0_epochs_DF.shape                               
g0_epochs_DF.head(3)

evoked_g0f=epochs_g0f.copy().average()
evoked_g0f.shapeN_events=len(epochs_g0f)#or events_gf.shape[0]
print(epochs_g0f)

info_ev=evoked_g0f.info
...
m0_epochs_data=np.squeeze(epochs_m0f.get_data())
m0_epochs_DF=pd.DataFrame(data=(m0_epochs_data[0:].T))
# g_epochs_DF.insert(0, 'time', epochs_gf.times)
m0_av=m0_epochs_DF.mean(axis=1)
m0_std=m0_epochs_DF.std(axis=1)
m0_sem=m0_epochs_DF.sem(axis=1)
...
m0_epochs_DF.iloc[:,:].plot(legend=None, ylim=[-2e-11,2e-11],title='magnetometer')
plt.show()
info_ev_m=evoked_m0f.info
evoked_m0f=mne.EvokedArray(np.array([m0_av]), info_ev_m, tmin=-0.2)

m0_av=-np.squeeze(evoked_m0f.filter(l_freq=l_freq,h_freq=h_freq).get_data())
...
# tg=int(sfreq_g*2)
# tfl=int(sfreq_m*2)
g_0ev_data=np.squeeze(evoked_g0f.get_data())
#aux_ev_data=np.squeeze(evoked_aux.get_data())*-1e-12
evoked_g0f.times.shape
...
m0_ev_data=np.squeeze(evoked_m0f.get_data())
# fl_ev_data.shape#
...
sfreq_gf = epochs_g0f.info['sfreq']
sfreq_mf = epochs_m0f.info['sfreq']
###############################
###### average of fft ################
...
freq_range_m0 = range(np.where(np.floor(freqs_m0f) == 1.)[0][0], np.where(np.ceil(freqs_m0f) == fmax - 1)[0][0])
psdsm0f_plot = 10 * np.log10(psds_m0f)
